...
impl <M> File<M> {
    pub fn new(db: Arc<SafeDb<M>>, path: &str) -> Self {
        Self {
            db,
            path: path.to_string(),
        }
...
impl <M> Dir<M> {
    pub fn new(db: Arc<SafeDb<M>>, path: String) -> Self {
        Self { db, path }
    }
}

impl <M> Debug for Dir<M> {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
...
impl <M:MemoryMap+'static> FileLike for File<M> {
    type Error = Error;
    ...
    fn write(&self, offset: u32, w_data: &[u8]) -> Result<u32, Self::Error> {
        ...
        if data.len() < offset as usize +w_data.len() {
            data.resize(offset as usize +w_data.len(), 0);
        }
        let d = &mut data.as_mut_slice()[offset as usize..offset as usize+w_data.len()];
        d.copy_from_slice(w_data);
...
impl <M:MemoryMap+'static> DirectoryLike for Dir<M> {
    ...
    fn create_dir(&self, name: &str) -> Result<(), Self::Error> {
        ...
        let ans = if insert.is_some() {
            ...
            // 根目录下创建目录
            let bucket = tx.create_bucket(unique_name)?;
            // 创建目录下的data文件
            // data文件存放目录下的子文件
            let bucket = bucket.create_bucket("data")?;
            bucket.put("..-d","")?;
            bucket.put(".-d","")?;
            Ok(())
        };
        ...
        }
        {
            ...
            let ans = if old.is_some() {
                if new.is_some() {
                    ...
                    let old_data = old_data.value().to_vec();
...
impl <M:MemoryMap+'static>  DbFileSystem<M> {
    /// init a new filesystem
    pub fn new(db: DB<M>) -> Self {
        Self {
            db: Arc::new(SafeDb(db)),
        }
    }
    pub fn root(&self) -> Arc<Dir<M>> {
        // 检查根目录是否存在
        let tx = self.db.0.tx(true).unwrap();
...
#[allow(unused)]
pub fn dbfs_test<M:MemoryMap+'static>(db: DB<M>) {
